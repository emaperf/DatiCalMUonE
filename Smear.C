...
,   int nEvents = -1
    ...
    TH1D *esumCalo=new TH1D ("esumCalo", "esumCalo", 150, 0, 150);
    TH1D *esumCaloFraction=new TH1D ("esumCaloFraction", "esumCaloFraction", 100, 0.5, 1.2);
    ...
    TH1D *nCryst=new TH1D("nCryst", "Cristalli illuminati", 50, 0, 50);
    TH1D *cDeposit=new TH1D("cDeposit", "cdeposit", 50, 0, 50);
	...
	for (int j=0; j<c_ncrystals; j++)
	{
        ...
        //dentro for istogrammi s1,s2,s1+s2, prima di if
	//fuori isto sums1 ,sums2...
	//cout<<ncrys<<"      "<<nTotcrys<<endl;
	p1->Fill(sumS1);
	p2->Fill(sumS2);
	p3->Fill(S1S2);
